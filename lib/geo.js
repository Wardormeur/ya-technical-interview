...
const checkParseFloat = (name, nb) => {
  const parsed = Number.parseFloat(nb);
  if (parsed.toString() !== nb) throw new RangeError(`${name} should be a number`);
  return parsed;
};

const parsePosition = ({ lat, long }) => {
  const parsedLat = checkParseFloat('lat', lat);
  const parsedLong = checkParseFloat('long', long);
